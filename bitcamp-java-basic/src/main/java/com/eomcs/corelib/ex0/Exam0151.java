...
public class Exam0151 {
  ...
  public static void main(String[] args) {
    ...
    Student s5 = new Student("김태희", 22, true);
    Student s6 = new Student("박민섭", 22, true);
    Student s7 = new Student("김하연", 22, true);
    Student s8 = new Student("최희진", 22, true);
    Student s9 = new Student("자바싫어", 22, true);
    Student s10 = new Student("유관손", 22, true);
    ...
    Iterator<Student> it = set.iterator(); // Iterator(반복자) 생성
    System.out.println("--------------");
    while (it.hasNext()) { // hasNext() : 데이터가 있으면 true 없으면 false
        Student s = it.next();
        System.out.printf("%s, %d, %s\n",
            s.name, s.age, s.working ? "재직중" : "실업중");
      } // next() : 다음 데이터 리턴
