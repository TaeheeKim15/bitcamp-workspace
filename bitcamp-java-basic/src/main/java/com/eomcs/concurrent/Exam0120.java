...
// 즉, 구현(프로그래밍)하기 쉽다
// 단점
// 1) 프로세스를 그대로 복제하기 때문에 프로세스가 사용하는 메모리도 그대로 복제된다.
// 2). 메모리 낭비가 심하다
// 3) 복제된 프로세스는 독립적이기 때문에 실행 종료할 때도 일일이 종료해야 한다.
// 
// 2) 멀티 스레딩
// - 특정 코드만 분리하여 실행한다.
// - 따라서 프로세스가 사용하는 메모리를 공유한다
//  장점
// 1) 프로세스으 ㅣ힙 메모리를 공유하기 때문에 메모리 낭비가 적다
// 2) 모든 스레드는 프로세스에 종속되기 때문에 프로세스를 종료하면
//     스레드도 자동 종료된다.
//
// 컨텍스트 스위칭
// - CPU를 실행시간을 쪼개 이 코드 저 코드를 실행할 때마다
// 실행 위치 및 정보를 저장하고 로딩하는 과정이 필요하다
// - 이 과정을 컨텍스트 스위칭이라 부른다
// 
// 스레드
// - 실이라는 뜻을 갖고 있다.
// 한 실행 흐름을 가리킨다
// 하나의 실은 끊지 않은 하나의 실행을 의미한다
//
// 스레드 생성
// 새 실을 만든다는 것이다
...
// 한 프로세스에 부여된 실행 시간을 다시 쪼개어 스레드에 나눠주는 방식이 아니다
