...
package com.eomcs.concurrent.ex05;
...
public class Exam0110 {
	...
	public static void main(String[] args) {
		...
		ATM 신촌 = new ATM("신촌", account);
		ATM 수지 = new ATM("수지", account);
		ATM 통영 = new ATM("통영", account);
		...
		// 이 예베에서는 여러 스레드가 동시에 호출하고
		// 같은 인스턴스의 변수의 값을 변경하는 메서드인 withdraw()가 
		// 임계구역이다
		//
		// 해결책
		// 한번에 한 스레드만이 크리티컬 섹션을 실행하도록 접근을 제한하면 된다
		// 주의
		// 동시에 여러 스레드가 같은 메모리에 접근하더라도
		// 값을 변경하느 것이 아니라 단순히 값을 조회하는 경우에는
		// 멀티 스레드 문제가 발생하지 않는다
		// 
		//
		// 세마포어(n) ; semaphore
		...
		// semaphore(1) 과 같다
		// 자바는 syncronized 키워드를 통해 뮤텍스를 사용할 수 있다.
	}
}
