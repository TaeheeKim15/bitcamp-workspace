...
public class Account {
	...
	// 한 번 에 한 스레드 만이 호출하도록 접근을 제한하고 싶다면
	// 메서드 전체를 동기화 블록으로 선언하라
	// 메서드 앞에 synchronized를 붙인다.
	// => 여러 스레드가 접근했을 때 문제가 발생하는 critical section 이 아닌데도
	...
	synchronized public long withdraw(long money) {
		...
		// 문제가 발생하는 코드를 "크리티컬 섹션(임계영역; critical section) 또는 크리티컬 리전
		// 2) 크리티컬 섹션에 동시에 접근하지 못하게 하는 기법을
		// 뮤텍스 || 세마포어(1)(semaphore)라 부른다
		// 3) 자바에서 뮤텍스를 구현하는 방법
